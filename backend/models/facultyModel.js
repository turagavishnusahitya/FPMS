...
const getFacultyById = async (faculty_id) => {
  const result = await pool.query(
    "SELECT * FROM faculty_users WHERE faculty_id = $1",
    [faculty_id]
  );
  return result.rows[0];
};
...
const findFacultyByIdAndCode = async (faculty_id, security_code) => {
  const result = await pool.query(
    "SELECT * FROM faculty_users WHERE faculty_id = $1 AND security_code = $2",
    [faculty_id, security_code]
  );
  return result.rows[0];
};

// Update the faculty password
const updateFacultyPassword = async (faculty_id, hashedPassword) => {
  await pool.query(
